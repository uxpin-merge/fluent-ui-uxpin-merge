...
import { Facepile as FFacepile, OverflowButtonType } from '@fluentui/react/lib/Facepile';
...
class Facepile extends React.Component {
  ...
  set() {
    ...
    this.setState({ personaList: configuredPersonas });
  }
  ...
  _getSelectedPersonaIndex(persona) {
    ...
    for (i = 0; i < this.state.personaList.length; i++) {
      ...
      if (initials === selectedInitials) return i + 1; //Use a 1-based index
    }
  }
  ...
  _getLinkedEmail(personaProps) {
    ...
    if (personaProps.email && personaProps?.email?.trim().length > 0) {
      ...
      linkedEmail = <Link value={personaProps.email} href={link ? link : ''} bold={false} italic={false} />;
    }
  ...
  _toggleIsCalloutVisible(overflowHoverIsShown) {
    overflowHoverIsShown
      ? this.setState({ overflowHoverIsShown: false })
      : this.setState({ overflowHoverIsShown: true });
  }
