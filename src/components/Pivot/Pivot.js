...
class Pivot extends React.Component {
  ...
  componentDidUpdate(prevProps) {
    if (prevProps.tabs !== this.props.tabs || prevProps.selectedIndex !== this.props.selectedIndex) {
      this.set();
    }
  }
  ...
  render() {
    ...
    for (i = 0; i < tabs.length; i++) {
      ...
      if (t?.text || t?.icon) {
        let tab = <PivotItem headerText={t.text} itemKey={t.key} key={t.key} itemIcon={t.icon} />;
    ...
    let key =
      this.state.selectedIndex < 1
        ? 1
        : this.state.selectedIndex > tabList.length
        ? tabList.length
    ...
    if (this.props.children) {
      ...
      if (childList.length && key - 1 < childList.length) {
        ...
        tabPanel = (
          <StackItem key={key} align={stretchAlign} grow={false}>
            {child}
          </StackItem>
        );
      }
