...
class Banner extends React.Component {
  ...
  render() {
    ...
    if (isCustom) {
      ...
      iconOrImg = <Image {...imgProps} />;
    } else {
      icnColor = UxpColors.getHexFromHexOrToken(icnColor);
      if (!icnColor) icnColor = '#000';

      iconOrImg = <Icon iconName={icnName} color={icnColor} size={icnSize} />;
    }
    ...
    return (
      ...
      >
        <StackItem tokens={iconStackTokens} verticalAlign={vertAlign}>
          {iconOrImg}
        </StackItem>

        <StackItem align={stretch} grow={3}>
          {mainContents}
        </StackItem>

        <StackItem>{dismissBtn}</StackItem>
      </Stack>
    );
  }
}
