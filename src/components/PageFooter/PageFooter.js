...
const confidentiality =
  'CONFIDENTIALITY NOTICE: This web site is intended only for the use of Company employees, and contains information that is privileged and confidential.';
...
class PageFooter extends React.Component {
  ...
  set() {
    ...
    this.setState({
      textColor:
        UxpColors.getHexFromHexOrToken(this.props.textColor) || UxpColors.getHexFromHexOrToken(defaultTextColor),
    });
  }
  ...
  componentDidUpdate(prevProps) {
    if (prevProps.links !== this.props.links || prevProps.textColor !== this.props.textColor) {
      this.set();
    }
  }
    ...
    if (this.props.links) {
      ...
      if (items && items.length) {
        for (var i = 0; i < items.length; i++) {
          ...
          if (linkInfo) linkList.push(linkInfo);
        } //for loop
      } //if items
    } //if props.links
  ...
  _configTextOrLink(textLinkInfo, textSize, addLinkDelimiter) {
    ...
    if (!textLinkInfo || !textSize) return '';
    ...
    let textSpan = <span style={fTextStyles}>{text}</span>;
    ...
    if (addLinkDelimiter) {
      ...
      linkDelimiter = <span style={delimiterStyle}>{' | '}</span>;
    }
  ...
  render() {
    ...
    if (this.props.line1Value) {
      ...
      line1 = (
        <StackItem>
          <Text styles={l1Styles} variant={defaultLine1TextSize}>
    ...
    if (l2Info) {
      ...
      line2 = <StackItem>{contents}</StackItem>;
    ...
    if (l3Info) {
      ...
      line3 = <StackItem>{contents}</StackItem>;
      showDivider = true;
    }
    ...
    if (this.state.links && this.state.links.length) {
      for (var i = 0; i < this.state.links.length; i++) {
        ...
        if (i + 1 < this.state.links.length) addLinkDelimiter = true;
    ...
    if (copyrightInfo) {
      ...
      copyrightElem = <StackItem>{contents}</StackItem>;
    }
    ...
    if (confidentialityInfo) {
      ...
      confidentialityElem = <StackItem>{contents}</StackItem>;
    }
