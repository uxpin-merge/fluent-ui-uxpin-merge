...
class TestControl extends React.Component {
  ...
  _getBorderStyle(edge) {
    ...
    if (this.props.borderEdge === borderNone) return 'none';
    ...
    if (this.props.children) {
      ...
      if (childList.length) {
        for (let i = 0; i < childList.length; i++) {
          ...
          let stack = <StackItem align={doStretch ? stretchAlign : hAlign}>{child}</StackItem>;
