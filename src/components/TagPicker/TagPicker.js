...
class TagPicker extends React.Component {
  ...
  set() {
    let suggestions = UXPinParser.parse(this.props.items).map((item, index) => ({
      name: item.text,
      key: index + 1,
    }));

    //We keep this duplicate to track selected tag indexes
    let items = suggestions.map((item) => ({
      name: item?.name,
      key: item?.key,
    }));
  ...
  componentDidUpdate(prevProps) {
    if (prevProps.items !== this.props.items || prevProps.selectedIndexes !== this.props.selectedIndexes) {
      this.set();
    }
  }
  ...
  parseSelectedIndexes(rawList, maxCount) {
    if (!rawList || rawList.trim().length === 0) return;
  ...
  filterItemsByText(filterText) {
    ...
    let filteredList = this.state.suggestionList.filter(function (item) {
  ...
  listContainsItem(tag, itemList) {
    if (!itemList || !itemList.length || itemList.length === 0) return false;

    if (!tag || !tag.name || tag.name.trim() === '') return false;
