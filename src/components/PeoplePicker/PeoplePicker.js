...
class PeoplePicker extends React.Component {
  ...
  componentDidUpdate(prevProps) {
    if (prevProps.selectedIndexes !== this.props.selectedIndexes || prevProps.persons !== this.props.persons) {
      this.set();
    }
  }
    ...
    if (this.props.persons) {
      ...
      if (items && items.length) {
        for (var i = 0; i < items.length; i++) {
          ...
          if (personInfo) people.push(personInfo);
        }
      }
    }
  ...
  filterPersonasByText(filterText) {
    ...
    let filteredList = this.state.suggestionList.filter(function (persona) {
  ...
  listContainsPersona(persona, personaList) {
    if (!personaList || !personaList.length || personaList.length === 0) return false;

    if (!persona || !persona.text || persona.text.trim() === '') return false;
  ...
  _onItemsChanged(items) {
    ...
    if (this.props.onChange) {
      ...
      for (i = 0; i < items.length; i++) {
        ...
        if (index > -1) indexList.push(index + 1);
      }
  ...
  render() {
    ...
    let ppLabel = (
      <Label {...this.props} required={this.props.required} disabled={this.props.disabled} htmlFor={ppID}>
        {ppLabelText}
      </Label>
    );
