...
class CardFooter extends React.Component {
  ...
  render() {
      ...
      if (childList && childList.length) {
        for (var i = 0; i < childList.length; i++) {
          ...
          let stack = (
            ...
            >
              <StackItem align={this.props.stretch ? 'stretch' : ''}>{child}</StackItem>
            </Stack>
          );
    ...
    return (
      <Stack tokens={stackTokens} horizontal={true} horizontalAlign={'start'} wrap={false} styles={topStackItemStyles}>
        {stackList}
      </Stack>
    );
  }
